...
3. Compile dist files (Development) :
    ...
    * Make your changes only in ./build Folder OR package.json OR ./dist/js/demo.js OR package-lock.json OR ./dist/js/Liberaciones/ OR ./docs OR in any html files which are necessary for contribution
    ...
    * `npm run production` (compile css/js files and test all Liberaciones are perfectly working fine, before creating a pull request)
